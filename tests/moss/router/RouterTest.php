...
class RouterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testMakeWithDefaultController()
    {
        ...
        $this->assertEquals(
            'http://test.com/router/foo/123/', $this->Router->make(
                null, array(
                           'foo' => 'foo',
                           'bar' => 123
                      )
    ...
    public function testMakeNormal()
    {
        $this->Router->match($this->mockRequest('router:foo:bar', '/router/foo/123/'));
        $this->assertEquals(
            'http://test.com/?controller=router_foo_bar&foo=foo&bar=123', $this->Router->make(
                'router:foo:bar', array(
                                       'foo' => 'foo',
                                       'bar' => 123
                                  ), true, false
            )
    ...
    public function testMakeRelative()
    {
        $this->Router->match($this->mockRequest('router:foo:bar', '/router/foo/123/'));
        $this->assertEquals(
            './router/foo/123/', $this->Router->make(
                'router:foo:bar', array(
                                       'foo' => 'foo',
                                       'bar' => 123
                                  ), false, true
    ...
    public function testMakeAbsoluteWithQuery()
    {
        $this->Router->match($this->mockRequest('router:foo:bar', '/router/foo/123/'));
        $this->assertEquals(
            'http://test.com/router/foo/123/?yada=yada', $this->Router->make(
                'router:foo:bar', array(
                                       'foo' => 'foo',
                                       'bar' => 123,
                                       'yada' => 'yada'
